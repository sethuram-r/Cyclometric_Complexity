...
class CodeComplexityWorker:
    ...
    def respond_to_request(self,key):
        ...
        return jsonify(worker_id =self.worker_id, commit_number = str(key), cyclometric_complexity  = cc_msg)
        #return str(post_msg)
...
def get(key):
    return worker.respond_to_request(key)


if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument('--workerID',type=int,default=1,help='ID to identify worker')
    ...
    worker = CodeComplexityWorker(FLAGS.workerID, worker_name, FLAGS.shost, FLAGS.sport)

       # run application with set parameters
    app.run(host=FLAGS.shost, port=FLAGS.sport)
